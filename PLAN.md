...
**M8 — Game-stage rewrite (2026-07-29) — complete.** Requested directly by the owner, outside the original milestone list (added here 2026-08-07 per OPEN-WORK.md §C4 — the code and commit history already reflected it as done, this milestone list didn't). Rewrote the board's presentation layer into a real coordinate-space stage: one fixed design-resolution layout scaled to the viewport via a single CSS transform, every tile absolutely positioned by computed x/y/rotation instead of document flow. Four steps, ~~each independently verified via Playwright across desktop/iPad viewports and all three `tileScale` presets~~ **(true of the one-off verification runs performed during the M8 session itself, but — corrected 2026-08-08, same stale-claim class as SPEC.md §5b item 6 — not currently reproducible: `packages/ui/e2e/board.spec.ts`, the only board-level Playwright coverage, had already gone stale from the M8 testid rename by the time it was written up here, then was removed entirely on `feat/landing-auth-persistence`; see OPEN-WORK.md §A11 for the full account and §D for why this matters beyond this one milestone entry)**: (1) the coordinate-space foundation; (2) a wood-rail/felt table look; (3) `motion`-driven `layoutId`-based tile-movement animation across zones — the direct payoff of this file's stable-tile-ID rule; (4) `@dnd-kit`-based drag-to-reorder with real keyboard operability, replacing the old bespoke pointer-drag mechanism. One known limitation, investigated but left unresolved per explicit owner direction ("not critical"): the human's own wall→hand draw doesn't get the travel animation (pops in at rest instead) — every other draw/discard/meld pairing animates correctly.
